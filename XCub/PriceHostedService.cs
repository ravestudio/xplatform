...
namespace XCub
{
    public class PriceHostedService : IHostedService, IDisposable
    {
        ...
        public PriceHostedService(IServiceScopeFactory scopeFactory, RabbitSender rabbitSender)
        {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            chinaQuoteSeq = Observable.Create<Quote>(observer =>
            {
                ...
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    var securityCodes = context.SecuritySet.Where(s => new string[] { "stock", "etf" }.Contains(s.Type) && s.Region == "China").Select(s => s.Code).ToList();

                    quoteList = context.QuoteSet
            ...
            usaQuoteSeq = Observable.Create<Quote>(async observer =>
            {
                ...
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    var securityCodes = context.SecuritySet.Where(s => new string[] { "stock", "etf" }.Contains(s.Type) && s.Region == "United States").Select(s => s.Code);
            ...
            moscowQuoteSeq = Observable.Create<MarketData>(async observer =>
            {
                ...
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    securityObj = context.SecuritySet.Where(s => s.Region == "Moscow").ToList();

                    var securityCodes = securityObj.Select(s => s.Code);

                    quoteList = context.QuoteSet.Where(q => securityCodes.Contains(q.symbol))
                    .OrderBy(q => q.symbol).ToList();
                }
